            ...
            $(window).scroll(function() {
                if(window.innerHeight + window.scrollY + 1 >= document.body.offsetHeight) {
                    setTimeout(function() {
                        ...
                        outerDiv.classList.add("col-xs-12");
                        outerDiv.classList.add("review-wrap");
                        outerDiv.classList.add("productDetail-review");

                        var innerDiv = "";
                        innerDiv += '<div class="col-md-3 review-inner">';
                        innerDiv += '<div class="reviewInner-img">';
						innerDiv += '<img src="${pageContext.request.contextPath}/resources/img/profile.png">';
                        innerDiv += '</div>';
                        innerDiv += '<div class="reviewInner-info">';
                        innerDiv += '<div>홍길동</div>';
                        innerDiv += '<div class="reviewInfo-star">';
                        innerDiv += '<span class="glyphicon glyphicon-star"></span>';
                        innerDiv += '<span class="glyphicon glyphicon-star"></span>';
                        innerDiv += '<span class="glyphicon glyphicon-star"></span>';
                        innerDiv += '<span class="glyphicon glyphicon-star"></span>';
                        innerDiv += '<span class="glyphicon glyphicon-star star-empty"></span>';
                        innerDiv += '</div>';
                        innerDiv += '</div>';
                        innerDiv += '</div>';
                        innerDiv += '<div class="col-md-9">';
                        innerDiv += '<form action="" class="review-content">';
                        innerDiv += '<p class="clearfix">맛있어요</p>';
                        innerDiv += '<button type="button" class="right btn btn-default" id="replyRegist"><span class="glyphicon glyphicon-remove"></span> 삭제</button>';
                        innerDiv += '<button type="button" class="right btn btn-default" id="replyRegist"><span class="glyphicon glyphicon-pencil"></span> 수정</button>';
                        innerDiv += '</form>';
                        innerDiv += '</div>';
                    
                        outerDiv.innerHTML = innerDiv;
                        
                        document.querySelector(".review-list").appendChild(outerDiv);
